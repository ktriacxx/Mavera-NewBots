...
let suspiciousRole = cezalıSettings.get(`suspicious`)
    ...
    if (!message.member.roles.cache.get(owner) && !message.member.hasPermission("ADMINISTRATOR")) return message.react(no)
   let deleted = args[0]
    if(!deleted) { message.inlineReply(`Lütfen bir sayı gir. \`.sil <sayı>\`  `) }
    message.channel.bulkDelete(deleted)
}

exports.conf = {enabled: true, guildOnly: false, aliases: ['clear', 'temizle', 'sil'], permLevel: 0}
