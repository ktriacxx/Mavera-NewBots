...
let vmuted = cezalıSettings.get(`vmuted`)
    ...
    if (!args[0]) return message.inlineReply(`Lütfen bir ID belirtiniz. \`.unjail @Mavera/ID\` `).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));

user.roles.remove(jailled)
message.react(yes);
}

exports.conf = {enabled: true, guildOnly: true, aliases: ['unjail', 'un-jail']}
exports.help = {name: 'Un-Jail'}