...
const footerSet = require("../../../../../Settings/Guard/Settings.json")
    ...
    if (!message.member.roles.cache.get(muteci) && !message.member.roles.cache.get(owner) && !message.member.hasPermission(8)) return message.react(no)
...
if(!user) return message.inlineReply(`Lütfen bir kullanıcı belirt. \`.mute @Mavera/ID\` `).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));
if(!args[1]) return message.inlineReply(`Lütfen bir zaman dilimi belirt. \`(\`1s/m/h/d/y\`)\` `).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));
...
warnSettings.add(`${user.id}_vmuteSayı`, 1) // mute yeme sayısı
warnSettings.add(`mutePuan.${user.id}`, -5) // mute yeme puanı
warnSettings.set(`${user.id}_muteDurum`, true) // mute durumu: true
...
message.inlineReply(embed.setDescription(`${user} kullanıcısı **${message.author.tag}** yetkilisi tarafından ${sebep} sebebiyle metin kanalları üzerinden susturuldu!`)).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));
user.roles.add(mutedRole)
moment.locale("tr")
client.channels.cache.find(a => a.name == "mute-log").send(embed.setTitle("Bir Kullanıcı Metin Kanalında Susturuldu!").setDescription(`
• Ceza ID: \`#${cezalandırılmaID}\` 
• Mutelenen Üye: : ${user} (**${user.user.id}**)
• Muteleyen Yetkili: ${message.author} (**${message.author.id}**)
• Başlangıç-Bitiş Tarihi: \`${muteatılma}\` - \`${mutebitiş}\`
• Mute Sebebi: [\`${sebep}\`]
`))

client.channels.cache.find(a => a.name == "penalties-log").send(`${no} ${user}: aldığınız **#${cezalandırılmaID}** numaralı chat-mute cezası alarak **${cezaPuan}** ceza puanına ulaştınız!`)

setTimeout(async () =>{
    user.roles.remove(mutedRole)
    user.send(`${yes} ${user}, **${message.guild.name}** adlı sunucumuzdaki "chat-mute" cezan bitti! Lütfen bir daha kural ihlali yapmamaya çalış.`)
    client.channels.cache.find(a => a.name == "mute-log").send(embed.setDescription(`${user} kullanıcısının metin kanalları üzerindeki cezası bitmiş bulunmaktadır. **#${cezalandırılmaID}** numaralı cezayı almış ve **${cezaPuan}** ceza puanına ulaşmış bulunmaktadır.`))
}, ms(sure))
        , ms(sure)}

exports.conf = {aliases: ['chatmute', 'chat-mute', 'cmute', 'c-mute', 'chatm', 'chat-m', 'mute'], permLevel: 0} 
exports.help = {name: "Chat-Mute"}