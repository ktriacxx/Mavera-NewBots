...
let vmuted = cezalıSettings.get(`vmuted`)
    ...
    if (!user) return message.inlineReply(`Lütfen bir kullanıcı belirtiniz. \`.unmute @Mavera/ID\` `).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));

user.roles.remove(muted)
message.react(yes);
}

exports.conf = {enabled: true, guildOnly: true, aliases: ['unmute', 'unchatmute', 'uncmute', 'un-cmute', 'un-mute', 'un-chat-mute', 'un-c-mute']}
exports.help = {name: 'Un-Mute'}