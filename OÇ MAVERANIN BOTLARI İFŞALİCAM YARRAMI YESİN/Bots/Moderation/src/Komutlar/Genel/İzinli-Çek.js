...
const footerSet = require("../../../../Settings/Guard/Settings.json")
    ...
    if (!victim.voice.channel) return message.inlineReply(`Yanına çekmek istediğin kullanıcı veya sen bir ses kanalında bulunmuyorsunuz.`).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));
    ...
    message.inlineReply(`${victim}, ${message.author} kullanıcısı sizi odasına çekmek istiyor. Cevabınız nedir?`).then(async(msg) => {
        msg.react(yes)
        msg.react(no)
        ...
        onay.on("collect", async() => {
            ...
            message.inlineReply(emebd.setDescription(`${yes} Kullanıcı sorunuzu kabul etti ve odanıza çektim.`)).then(m => m.delete({ timeout: 7000 }))})

        red.on("collect", async() => {
            await msg.reactions.removeAll()
            message.inlineReply(embed.setDescription(`${no} Kullanıcı sorunuzu reddettiği için odanıza çekemedim.`)).then(m => m.delete({ timeout: 7000 }))
})})}

exports.conf = {enabled: true, guildOnly: true, aliases: ['izinliçek', 'izinli-çek', 'içek', 'i-çek'], permLevel: 0}
