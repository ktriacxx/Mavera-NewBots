...
//////// Jail, Mute, Voice-Mute yiyen kullanıcılar olduğu zaman kayıt işlemleri durdurulacak. Bu EKLENECEK !
    ...
    if(!user) return message.inlineReply(`Lütfen bir kullanıcı belirtiniz! \`.e @Mavera/ID isim yaş\` `).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));
    if(!nick || !age) return message.inlineReply(`Lütfen bir isim yaş belirtiniz! \`.e @Mavera/ID isim yaş\` `).then(x => x.delete({ timeout: 5000 }) && message.delete({ timeout: 5000 }));
...
// database load start ###
await registerSettings.add(`kayıt.${message.author.id}`, 1) // start msg.author database
statSettings.add(`coin.${message.author.id}`, 5) // tünelde awp var awq koş
registerSettings.add(`erkek.${message.author.id}`, 1) // end msg.author database
registerSettings.add(`${user.id}.kayıt`, 1) // start user database
registerSettings.add(`kayıt.${user.id}.sayı`, 1)
registerSettings.push(`veri_${user.id}`, { userID: user.id, isim: nick, yas: age, tag: serverTag, rol: erkek1, reason: "Erkek" }) // end user database
 // end user database
// database info end ###
...
client.channels.cache.get(chat).send(`:tada: ${user} aramıza hoşgeldin! Seninle birlikte **${message.guild.memberCount}** kişiye ulaştık!`)

}

exports.conf = {enabled: true, guildOnly: true, aliases: ['e', 'erkek', 'man']}
